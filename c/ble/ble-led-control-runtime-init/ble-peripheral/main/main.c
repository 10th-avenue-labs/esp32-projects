...
int gatt_svc_init(void) {
    ...
    struct ble_gatt_chr_def led_char = {
        .uuid = &led_chr_uuid.u,
    ...
    struct ble_gatt_chr_def *dynamic_chars = malloc(sizeof(struct ble_gatt_chr_def) * 2);
    dynamic_chars[0] = led_char;
    dynamic_chars[1] = (struct ble_gatt_chr_def) {0};

    struct ble_gatt_svc_def serv = {
        .type = BLE_GATT_SVC_TYPE_PRIMARY,
        .uuid = &auto_io_svc_uuid.u,
        .characteristics = dynamic_chars
    };

    struct ble_gatt_svc_def *dynamic_gatt_svr_svcs = malloc(sizeof(struct ble_gatt_svc_def) * 2);
    dynamic_gatt_svr_svcs[0] = serv;
    dynamic_gatt_svr_svcs[1] = (struct ble_gatt_svc_def) {0};

    // 2. Update GATT services counter
    rc = ble_gatts_count_cfg(dynamic_gatt_svr_svcs);
    if (rc != 0) {
        return rc;
    }

    // 3. Add GATT services
    rc = ble_gatts_add_svcs(dynamic_gatt_svr_svcs);
    if (rc != 0) {
        return rc;
    }
