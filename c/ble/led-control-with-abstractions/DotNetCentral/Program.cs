...
if (service == null) {
    ...
    Console.WriteLine($"Could not find LED service with ID '{serviceUuid}'. Instead found the following:");
...
// This doesn't always get the most up-tp-date flags from the device
// Sometimes this will grab a cached value or something somewhere
// In any case, these flags can't be completely trusted
var flags = await characteristic.GetFlagsAsync();
Console.WriteLine($"Flags count: {flags.Length}");
...
await 10.TimesAsync(async () => {
    Console.WriteLine($"Writing '{count}' to characteristic.");
    await characteristic.WriteValueAsync([Convert.ToByte(count)], new Dictionary<string, object>());
