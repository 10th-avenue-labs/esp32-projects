...
// TODO: Remove these defines in favor of some variable
#define TAG "NimBLE_Beacon"
#define BLE_GAP_APPEARANCE_GENERIC_TAG 0x0200
#define BLE_GAP_URI_PREFIX_HTTPS 0x17
#define BLE_GAP_LE_ROLE_PERIPHERAL 0x00

// TODO: Remove these static variables in favor of some instance variable
...
int gap_init(const char *device_name, uint16_t device_appearance) {
    // Local variables to be reused
    int response = 0;
    ...
    response = ble_svc_gap_device_name_set(device_name);
    if (response != 0) {
    ...
    response = ble_svc_gap_device_appearance_set(device_appearance);
    if (response != 0) {
 ...
 */
int gatt_svc_init(struct ble_gatt_svc_def *services) {
    ...
    response = ble_gatts_count_cfg(services);
    if (response != 0) {
        return response;
    }

    // 3. Add GATT services
    response = ble_gatts_add_svcs(services);
    if (response != 0) {
        return response;
    }
...
bool ble_advertiser_init(BleAdvertiser *bleAdvertiser) {
    ...
    // Initialize the Generic Attribute Profile (GAP)
    response = gap_init(bleAdvertiser->device_name, bleAdvertiser->device_appearance);
    if (response != 0) {
    ...
    struct ble_gatt_svc_def *services = create_ble_services(bleAdvertiser->services, bleAdvertiser->service_count);

    // Initialize the Generic ATTribute Profile (GATT) server <- I know, it's a bad acronym
    response = gatt_svc_init(services);
    if (response != 0) {
...
void ble_advertiser_advertise(BleAdvertiser *bleAdvertiser) {
    ...
    ESP_LOGI(TAG, "nimble host task has been started by %s", bleAdvertiser->device_name);
